...
class _MyProfilePageState extends State<MyProfilePage> {
  ...
  Future<void> fetchUserData() async {
    try {
      final response = await http
          .get(Uri.parse('http://192.168.1.9:3000/profile/${widget.userId}'));
  ...
  Future<void> checkHomemakerStatus() async {
    try {
      final response = await http.get(Uri.parse(
          'http://192.168.1.9:3000/profile/${widget.userId}/isHomemaker'));
      if (response.statusCode == 200) {
  ...
  Future<void> fetchAvailabilityStatus() async {
    try {
      final response = await http.get(Uri.parse(
          'http://192.168.1.9:3000/profile/${widget.userId}/availability'));
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        setState(() {
              ...
              onPressed: () async {
                ...
                try {
                  final response = await http.delete(
                    Uri.parse(
                        'http://192.168.1.9:3000/profile/${widget.userId}'),
                  );
                  if (response.statusCode == 200) {
  ...
  Future<void> logoutBackend() async {
    try {
      final response = await http.post(
        Uri.parse('http://192.168.1.9:3000/logout'),
      );
    ...
    return Scaffold(
      ...
      IconButton(
        ...
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => OrderPage(chefId:widget.userId)),
          );
        },
      ),
    ],

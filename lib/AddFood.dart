...
class _AddFoodState extends State<AddFood> {
  ...
  void editFood(
    ...
    print(foodList);
    final url = Uri.parse(
        'http://192.168.1.9:3000/food-items/$id'); // Use the received id
  ...
  void deleteFoodItem(String? id) async {
    ...
    if (deleteConfirmed != null && deleteConfirmed) {
      final url = Uri.parse('http://192.168.1.9:3000/food-items/$id');
...
void initState() {
  super.initState();
  print(widget.chefId);
  scheduleAccountDeletion(widget.chefId);
  ...
  final url = Uri.parse('http://192.168.1.9:3000/food-items?chefId=$chefId');
  final response = await http.get(url);

  if (response.statusCode == 200) {
    List<dynamic> data = jsonDecode(response.body);
    print(data); // Print response data to debug
   ...
   void scheduleAccountDeletion(String chefId) {
    // Schedule account deletion after 2 minutes
    Timer(Duration(minutes: 1), () async {
      try {
        // Check if food items are added by the homemaker
        // If not, delete the account
        // Make an HTTP request to your backend to check if food items are added
        // If not, delete the account
        final response = await http.get(
          Uri.parse('http://192.168.1.9:3000/food-items?chefId=$chefId'),
        );
    ...
    if (responseData.isEmpty) {
        ...
        final deleteResponse = await http.delete(
          Uri.parse('http://192.168.1.9:3000/profile/${widget.chefId}'),
        );
        ...
        if (deleteResponse.statusCode == 200) {
          ...
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => LoginPage(), // Navigate to your logout page
            ),
          );
        } else {

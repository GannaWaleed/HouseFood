...
class _CartPageState extends State<CartPage> {
  ...
  Future<void> fetchCartItems() async {
    try {
      ...
      final url =
          Uri.parse('http://192.168.1.9:3000/cart?userId=${widget.userId}');
  ...
  Future<FoodItem> fetchFoodItem(String foodItemId) async {
    try {
      final url = Uri.parse('http://192.168.1.9:3000/food-items/$foodItemId');
      final response = await http.get(url);
      if (response.statusCode == 200) {
  ...
  void increaseQuantity(String id) async {
    try {
      final url = Uri.parse('http://192.168.1.9:3000/cart/increase/$id');
  ...
  void decreaseQuantity(String id) async {
    try {
      final url = Uri.parse('http://192.168.1.9:3000/cart/decrease/$id');
      final response = await http.patch(url);
      if (response.statusCode == 200) {
  ...
  Future<void> deleteCartItem(String id) async {
    try {
      ...
      final url = Uri.parse('http://192.168.1.9:3000/cart/$id');
  ...
  Future<void> placeOrder() async {
    ...
    // print('Sending order data:', orderData); // Print order data before sending

    final response = await http.post(
      Uri.parse('http://192.168.1.9:3000/orders'),
    ...
    if (response.statusCode == 201) {
      ...
      final orderId = responseData['_id']; // Assuming orderId is stored in the response
      final chefId = _cartItems.isNotEmpty ? _cartItems.first.foodItem.chefId : '';
      clearCart();

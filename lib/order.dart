...
class _OrderPageState extends State<OrderPage> {
  ...
  Future<void> fetchOrders() async {
    final response = await http.get(
        Uri.parse('http://192.168.1.8:3000/orders/${widget.chefId}/pending'));
  ...
  Future<Map<String, dynamic>?> fetchUserDetails(String userId) async {
    final response =
        await http.get(Uri.parse('http://192.168.1.8:3000/register/$userId'));
    if (response.statusCode == 200) {
  ...
  Future<List<Map<String, dynamic>>?> fetchFoodItemDetails(String foodItemId) async {
  final response = await http.get(Uri.parse('http://192.168.1.8:3000/food-items/$foodItemId'));
  if (response.statusCode == 200) {
  ...
  builder: (context, AsyncSnapshot<List<Map<String, dynamic>>?> foodItemSnapshot) {
    if (foodItemSnapshot.connectionState == ConnectionState.waiting) {
      ...
      if (foodItemDetails != null && foodItemDetails.isNotEmpty) {
        ...
        return Text(
          '$name - Price: $price, Quantity: $quantity, Total Price: ${price * quantity}',
        );
      } else {
  ...
  Future<void> acceptOrder(String orderId) async {
    final response = await http.put(
      Uri.parse('http://192.168.1.8:3000/orders/$orderId'),
